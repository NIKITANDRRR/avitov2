...
def fmt_rating(n) -> str:
    """Форматирование рейтинга (без разделителей тысяч)."""
    if n is None:
        return "N/A"
    return f"{n:.2f}"
...
def main():
    ...
    try:
        with engine.connect() as conn:
            ...
            # (список статический — имена берутся только из него, не из пользовательского ввода)
            for table in tables:
                try:
            ...
            print(f"  Средний рейтинг               : {fmt_rating(row['avg_rating'])}")
